...
class RAGSystem:
    ...
    def index_documents(
        ...
        *,
        ...
        Args:
            ...
            base_metadata: applied to *every* chunk
            per_chunk_metadata: list aligned with `chunks`, lets you say row_index=0,1,2,...
        """
    ...
    def _batch_calculate_similarities(
        ...
        """
        Returns a 1D array of cosine similarities: shape (num_docs,)
        """
        # ensure 1D
    ...
    def save_index(self, dir_path: str) -> None:
        """
        Save FAISS index + metadata (and optionally embeddings) to disk.
        """
        ...
        # 3. (optional) save embeddings if you kept them as numpy
        if self.document_embeddings is not None:
            # ensure numpy
            if isinstance(self.document_embeddings, np.ndarray):
                arr = self.document_embeddings
            else:
                # in case it's a torch tensor from old code
                import torch
                if isinstance(self.document_embeddings, torch.Tensor):
                    arr = self.document_embeddings.detach().cpu().numpy()
                else:
                    arr = np.array(self.document_embeddings, dtype="float32")

            np.save(os.path.join(dir_path, "embeddings.npy"), arr)

    def load_index(self, dir_path: str) -> None:
        """
        Load FAISS index + metadata (and optionally embeddings) from disk.
        """
